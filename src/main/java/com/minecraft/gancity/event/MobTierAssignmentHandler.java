...
public class MobTierAssignmentHandler {
    ...
    private static boolean isLivestockOverhaulLoaded() {
        if (livestockOverhaulLoaded == null) {
            livestockOverhaulLoaded = isModLoaded("livestock_overhaul");
        }
    ...
    private static boolean isPetOverhaulLoaded() {
        if (petOverhaulLoaded == null) {
            petOverhaulLoaded = isModLoaded("petoverhaul");
        }
     ...
     */
    /**
     * Fabric hook: called when an entity is loaded into a ServerLevel.
     */
    ...
    public static TacticTier getTierFromMob(Mob mob) {
        CompoundTag data = ((PersistentDataHolder) mob).adaptivemobai$getPersistentData();
    ...
    private static void addEliteTeamBehavior(Mob mob) {
        ...
        ((MobGoalSelectorAccessor) mob).adaptivemobai$getGoalSelector().addGoal(2, new ElitePackCoordinationGoal(mob));
    }
    ...
    public static boolean hasTier(Mob mob) {
        return ((PersistentDataHolder) mob).adaptivemobai$getPersistentData().getBoolean(TIER_ASSIGNED_TAG);
    }
