...
**Symptom**: Game hangs during early mod initialization, log shows:
```
[main/INFO]: Compatibility level set to JAVA_17
```
Then freezes with no crash report.

**Cause**: Static mod-detection calls during class initialization (too early in loader/bootstrap).

**Fix**: **RESOLVED in v1.1.80+**
- Upgraded to lazy-loaded mod detection
...
**A. Verify `fabric.mod.json`**:
Ensure `fabric.mod.json` includes the mixin config:
```json
{
  "mixins": [
    "adaptivemobai.mixins.json"
  ]
}
```
